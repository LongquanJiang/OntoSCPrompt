...
def check_consistency(dataset, tokenizer, output):
    ...
    for ex in dataset:
        ...
        try:
            ...
            # assert normalized_sparql_1 == query
            #
            # output.write(f"id:{ex['uid']}\n")
            # output.write(f"original:\t{ex['query']}\n")
            # output.write(f"normalized:\t{query}\n")
...
if __name__ == '__main__':
    ...
    lcquad_train = load_dataset('./src/datasets/lcquad/lcquad.py', split='train')
    lcquad_test = load_dataset('./src/datasets/lcquad/lcquad.py', split='test')
    ...
    output = open("./consistency_check_outputs/lcquad.txt", "w")
    train_structures= check_consistency(lcquad_train, tokenizer, output)
    test_structures = check_consistency(lcquad_test, tokenizer, output)
    output.close()
