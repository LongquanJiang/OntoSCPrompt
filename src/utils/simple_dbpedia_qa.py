...
def simple_dbpedia_qa_get_input(
        question: str,
...
def simple_dbpedia_qa_get_target(
        query: str,
        db_id: str,
...
def simple_dbpedia_qa_add_serialized_schema(
        ex: dict,
        mode: Optional[str],
        ontologies: dict,
...
def simple_dbpedia_qa_pre_process_function(
        batch: dict,
        mode: Optional[str],
        data_training_args: DataTrainingArguments,
    ...
    if data_training_args.use_decomposition:
        ...
        for kb, qid, question, query, serialized_schema, structure_0, content_0 in zip(batch["kb"],
            ...
            input_str = simple_dbpedia_qa_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)

            if data_training_args.stage == "structure":
                inputs.append(data_training_args.schema_serialization_with_prompt + ' | ' + input_str)
                target = simple_dbpedia_qa_get_target(
                    query=structure,
                    db_id=kb,
        ...
        inputs = [
            simple_dbpedia_qa_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)
            for question, serialized_schema in zip(batch["question"], batch["serialized_schema"])
        ]
        targets = [
            simple_dbpedia_qa_get_target(
                query=query,
                db_id=kb,
                target_with_db_id=data_training_args.target_with_db_id,
            )
...
class SimpleDBpediaQATrainer(Seq2SeqTrainer):
