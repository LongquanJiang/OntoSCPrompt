...
from .dataset import serialize_schema, decompose, encode, decode, combine_SC
from transformers.tokenization_utils_base import PreTrainedTokenizerBase
from .args import *

tttt = []
def webqsp_get_input(
        question: str,
...
def webqsp_add_serialized_schema(
        ex: dict,
        mode: Optional[str],
        ontologies: dict,
        data_training_args: DataTrainingArguments
) -> dict:
    ontology = ontologies["webqsp"]
...
def webqsp_pre_process_function(
        batch: dict,
        mode: Optional[str],
        data_training_args: DataTrainingArguments,
    ...
    if data_training_args.use_decomposition:
        ...
        for kb, qid, question, query, serialized_schema, structure_0, content_0 in zip(batch["kb"],
            ...
            input_str = webqsp_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)

            if data_training_args.stage == "structure":
                inputs.append(data_training_args.schema_serialization_with_prompt + ' | ' + input_str)
                target = webqsp_get_target(
                    query=structure,
                    db_id=kb,
        ...
        inputs = [
            webqsp_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)
    ...
    for model_input, label_id in zip(model_inputs["input_ids"], labels["input_ids"]):
        input_tokens = tokenizer.convert_ids_to_tokens(model_input, skip_special_tokens=True)
        label_tokens = tokenizer.convert_ids_to_tokens(label_id, skip_special_tokens=True)
        #print(input_tokens)
        #print(label_tokens)

    if "WebQTrn-3248" in batch["uid"]:
        print(tokenizer.convert_ids_to_tokens(tokenizer(batch["WebQTrn-3103"]["structure"])), skip_special_tokens=True)
...
class WebQSPTrainer(Seq2SeqTrainer):
    def _post_process_function(
        ...
        for meta, p, d in zip(metas[0:5], predictions[0:5], decoded_label_ids[0:5]):
            print("subgraph: ", meta["subgraph"])
            print("pred: ", p)
            print("deco: ", d)
            print("\n")
        ...
        if self.stage == "content":
            ...
            with open(f"{self.args.output_dir}/content.json", "w") as f:
                json.dump(
                    [dict(**{"uid": meta["uid"]}, **{"subgraph": meta["subgraph"]}, **{"prediction": prediction},
