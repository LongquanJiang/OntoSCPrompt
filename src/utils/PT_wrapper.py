...
class PromptWrapper(nn.Module):
    ...
    def build_inputs(self, input_ids, attention_mask, labels=None):
        ...
        prompt_length = self.prompt_head.size(0) + self.prompt_mid1.size(0) + self.prompt_mid2.size(
            0) + self.prompt_tail.size(0)
        ...
        for idx0 in range(inputs_embeds.shape[0]):
            ...
            extend_embeds.append(torch.cat(
                [self.prompt_head, inputs_embeds[idx0][:mid1], self.prompt_mid1, inputs_embeds[idx0][mid1:mid2],
                 self.prompt_mid2, inputs_embeds[idx0][mid2:end], self.prompt_tail, inputs_embeds[idx0][end:]], 0))
    ...
    @torch.no_grad()
    def generate(self, input_ids=None, attention_mask=None, structures=None, ontologies=None, observed_num_examples=None, **kwargs):
