...
def dblp_qald_add_serialized_schema(
        ex: dict,
        mode: Optional[str],
        ontologies: dict,
        data_training_args: DataTrainingArguments
) -> dict:
    ontology = ontologies["dblp_qald"]

    ent2label = {ent["qid"].split(":")[1]: ent["label"] for ent in ontology["entities"]}
...
def add_whitespace(content):
    new_content = content.replace("pid:", "pid: ").replace(
        "rec:", "rec: ").replace(
        "dblp:", "dblp: ").replace(
        "rdf:", "rdf: ").replace(
        "rdfs:", "rdfs: ").replace(
...
def dblp_qald_pre_process_function(
        batch: dict,
        mode: Optional[str],
        data_training_args: DataTrainingArguments,
    ...
    if data_training_args.use_decomposition:
        ...
        for kb, qid, question, query, serialized_schema, structure_0, content_0 in zip(batch["kb"],
            ...
            input_str = dblp_qald_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)

            if data_training_args.stage == "structure":
                input_str = data_training_args.schema_serialization_with_prompt + '| ' + input_str
                inputs.append(input_str)
                target = dblp_qald_get_target(
                    query=structure,
                    db_id=kb,
        ...
        inputs = [
            dblp_qald_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)
            for question, serialized_schema in zip(batch["question"], batch["serialized_schema"])
        ]
        targets = [
            dblp_qald_get_target(
                query=query,
                db_id=kb,
                target_with_db_id=data_training_args.target_with_db_id,
            )
...
class DblpQALDTrainer(Seq2SeqTrainer):
        ...
        if self.stage == "content":
            ...
            if os.path.exists(hypotheses_path):
                ...
                with open(self.structure_path) as f:
                    info = json.load(f)
                    for item in info:
                        pred_structures.append(item["prediction"])

                for pred_content, pred_structure in zip(predictions, pred_structures):
                    final_pred_sparql = decode(combine_SC(content=remove_whitespace(pred_content), structure=pred_structure))
                    final_pred_sparqls.append(final_pred_sparql)
    ...
    def _compute_metrics(self, eval_prediction: EvalPrediction) -> dict:
        ...
        if self.stage == "structure":
            ...
            eval_metric_sparql = self.metric.compute(predictions=final_pred_sparqls, references=gold_sparqls)["exact_match"]
