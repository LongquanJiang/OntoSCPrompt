...
def lcquad_add_serialized_schema(
        ex: dict,
        mode: Optional[str],
        ontologies: dict,
    ...
    ontology = ontologies[ex[data_training_args.level_ontology]]
    ...
    # special process only for lcquad1
    # new_rel2label = {}
    # new_concept2label = {}

    # for k1, v1 in rel2label.items():
    #     splited = "".join([(" " + i if i.isupper() else i) for i in k1]).strip().split()
    #     new_k1 = ".".join(splited)
    #     new_v1 = " ".join(splited)
    #     new_rel2label.update({new_k1: new_v1})

    # for k2, v2 in concept2label.items():
    #     splited = "".join([(" " + i if i.isupper() else i) for i in k2]).strip().split()
    #     new_k2 = ".".join(splited)
    #     new_v2 = " ".join(splited)
    #     new_concept2label.update({new_k2: new_v2})
    ...
    serialized_schema = serialize_schema(
        ...
        db_entities=[ent for ent in ex["entities"]] if not ent2label else [],
...
def add_whitespace(content):
    new_content = content.replace("dbr:", "dbr: ").replace(
        "dbo:", "dbo: ").replace(
        "dbp:", "dbp: ").replace(
        "rdf:", "rdf: ").replace(
        "rdfs:", "rdfs: ").replace(
...
def lcquad_pre_process_function(
        batch: dict,
        mode: Optional[str],
        data_training_args: DataTrainingArguments,
    ...
    if data_training_args.use_decomposition:
        ...
        for kb, qid, question, query, serialized_schema, structure_0, content_0 in zip(batch["kb"],
            ...
            input_str = lcquad_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)

            if data_training_args.stage == "structure":
                inputs.append(data_training_args.schema_serialization_with_prompt + ' | ' + input_str)
                target = lcquad_get_target(
                    query=structure,
                    db_id=kb,
        ...
        inputs = [
            lcquad_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)
    ...
    for input_, target_, model_input, label_id in zip(inputs, targets, model_inputs["input_ids"], labels["input_ids"]):
...
class LCQuADTrainer(Seq2SeqTrainer):
